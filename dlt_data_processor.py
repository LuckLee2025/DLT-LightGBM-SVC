...
TXT_DATA_URL = 'http://data.17500.cn/dlt_asc.txt'
    ...
    try:
        ...
        for row in table.find_all('tr')[1:]:  # 跳过表头
            ...
            try:
                ...
                # 提取前区和后区
                front_balls_str = cells[1].text.strip().replace(" ", ",")
                back_balls_str = cells[2].text.strip().replace(" ", ",")

                # 验证号码格式
                front_numbers = [int(x) for x in front_balls_str.split(',')]
                back_numbers = [int(x) for x in back_balls_str.split(',')]
                if len(front_numbers) != 5 or len(back_numbers) != 2:
                    continue
                if not all(1 <= x <= 35 for x in front_numbers) or not all(1 <= x <= 12 for x in back_numbers):
                    continue
...
def fetch_full_data_from_txt(url: str = TXT_DATA_URL) -> list:
    ...
    """
    logger.info(f"正在从TXT文件源 ({url}) 下载大乐透全量数据...")
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
    }
    try:
...
def parse_txt_data(data_lines: list) -> list:
    ...
    for line in data_lines:
        ...
        if len(fields) < 9:
            continue  # 忽略格式不正确的行
        try:
            # 数据格式: [期号, 日期, 前1, 前2, 前3, 前4, 前5, 后1, 后2]
            qihao = fields[0]
            date = fields[1]
            front_balls = ",".join(fields[2:7])
            back_balls = ",".join(fields[7:9])
            # 基础验证
            if qihao.isdigit() and len(date.split('-')) == 3:
                parsed_data.append([qihao, date, front_balls, back_balls])
        except IndexError:
...
if __name__ == "__main__":
    ...
    txt_data_dicts = [{'期号': r[0], '日期': r[1], '前区': r[2], '后区': r[3]} for r in txt_parsed_list]
